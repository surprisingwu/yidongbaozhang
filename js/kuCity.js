    ...
    $.fn.kuCity = function(options) {
        ...
        target.on('focus', function(e) {
            ...
            kucity.resultct.on('click', 'li', function() {
                var targetVal = $(this).find('.name').text();
                $(".addFormPage").css("display","block");
                $(".selectPositionWraper").css("display","none");
                //kucity.target.val(targetVal);
                if (positionFlag) {
                    $("#placeOfDeparture").val(targetVal);
                }else{
                ...
                $(".letterWraper").show();
                $("#searchBox").val("");
                $("#searchBox").animate({"left":"0px"},500);
                $(".turnBackToInputForm").show();
                $("body").css({
                    "overflow":"auto",
                    "position":"relative"
                })
                $(".searchBoxCancelBtn").hide();
                positionFlag = true;
